<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="Role" width="100px">
        <template slot-scope="{row}">
...
<script>
import { fetchList, fetchPv, createPersonnel, updatePersonnel, deletePersonnel } from '@/api/personnel'
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          createPersonnel(this.temp).then(() => {
    ...
    handleDelete(row) {
      this.$confirm('确认删除这条记录吗？', '提示', {
        ...
        deletePersonnel(row.id).then(response => {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['timestamp', 'name', 'type', 'role']
        const filterVal = ['timestamp', 'name', 'type', 'role']
